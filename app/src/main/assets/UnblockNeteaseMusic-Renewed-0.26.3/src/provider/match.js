...
const match = (id, source, data) => {
	...
	return find(id, data)
		.then((info) => {
			meta = info;
			return Promise.all(candidate.map(name => providers[name].check(info).catch(() => Promise.resolve())))
		})
		.then((songs) => {
			if (!Array.isArray(songs)) songs = new Array(songs)
			songs = songs.filter(song => song && song.url)
			return Promise.all(songs.map(song => check(song.url, song.weight)))
		})
		.then((songs) => {
			if (!Array.isArray(songs)) songs = new Array(songs)
